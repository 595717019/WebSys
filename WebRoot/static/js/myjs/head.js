...
function online() {
  if (window.WebSocket) {
    ...
    websocket.onmessage = function(msg)// 消息接收
    {
...
function hf(b) {
  ...
  if (b == "skin-2") {
    ...
    $(".ace-nav > li:not(:last-child)").addClass("white-pink")
        .find('> a > [class*="icon-"]').addClass("pink").end().eq(0)
    ...
    $(".ace-nav > li:not(:last-child)").removeClass("white-pink")
        .find('> a > [class*="icon-"]').removeClass("pink").end().eq(0)
        .find(".badge").removeClass("badge-warning");
  }
  if (b == "skin-3") {
