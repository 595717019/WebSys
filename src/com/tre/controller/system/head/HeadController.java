package com.tre.controller.system.head;
...
import com.tre.service.system.appuser.AppuserService;
import com.tre.service.system.user.UserService;
import com.tre.util.AppUtil;
import com.tre.util.Const;
import com.tre.util.PageData;
import com.tre.util.SmsUtil;
import com.tre.util.Tools;
import com.tre.util.Watermark;
import com.tre.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
                    ...
                    try {
                        for (int i = 0; i < arrTITLE.length; i++) {
                            if (Tools.checkEmail(arrTITLE[i])) { // 邮箱格式不对就跳过
                                SimpleMailSender.sendEmail(strEM[0], strEM[1], strEM[2], strEM[3], arrTITLE[i], TITLE,
    ...
    public ModelAndView saveSys2() throws Exception {
        ...
        Tools.writeFile(Const.FWATERM, pd.getString("isCheck1") + ",fh," + pd.getString("fcontent") + ",fh,"
                + pd.getString("fontSize") + ",fh," + pd.getString("fontX") + ",fh," + pd.getString("fontY")); // 文字水印配置
    ...
    public ModelAndView saveSys3() throws Exception {
        ...
        Tools.writeFile(Const.WEBSOCKET, pd.getString("WIMIP") + ",fh," + pd.getString("WIMPORT") + ",fh,"
                + pd.getString("OLIP") + ",fh," + pd.getString("OLPORT")); // websocket配置
        mv.addObject("msg", "OK");
        mv.setViewName("save_result");
        return mv;
    }
